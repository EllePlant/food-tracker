...
import example.data.UserException;
...
public class LoginActivity extends AppCompatActivity {
    ...
    private void onRegisterButtonClick() {
        ...
        boolean isPasswordValid = password.matches("^(?=.*[a-z])(?=.*[A-Z])(?=.*\\d)(?=.*[^\\da-zA-Z]).{8,}$");
        ...
        if (!isPasswordValid) {
            // Show an error message
            Toast.makeText(LoginActivity.this, "Please input a stronger password", Toast.LENGTH_SHORT).show();
    ...
    private void onLoginButtonClick(){
        ...
        if(isRegistering){
            ...
            boolean isNameValid = name.matches("^[a-zA-Z\\-]+\\s?[a-zA-Z\\-]*$");
            if (!isNameValid) {
                // Show an error message
                Toast.makeText(example.application.LoginActivity.this, "Invalid name format", Toast.LENGTH_SHORT).show();

                // Enable the name EditText
